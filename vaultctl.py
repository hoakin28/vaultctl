...
def read_args():
    ...
    parser.add_argument("--load", help="Write secrets from file, file must be key:value for secrets, for policies /path/../../* read:list format with extension .sec", type=validate_file)
    ...
    parser.add_argument("--version", help="Display Vaultctl Version", action='store_true')
...
def validate_file(arg, file=re.compile(r".*\.sec")):
    if not file.match(arg):
        raise argparse.ArgumentTypeError("File must end with .sec extension")
    return arg


def version():
    print("Vaultctl v1.0.0")
    exit(0)
...
def read_config():
    ...
    if os.path.isfile("/etc/vaultctl.conf"):
        try:
            with open("/etc/vaultctl.conf", "r+") as config_file:
                config.read_file(config_file)
        except IOError as error:
            print(error)
            exit(1)
    else:
        print("No config file found, please create one as /etc/vaultctl.conf or use --url --token as arguments")
        exit(1)
    return config
